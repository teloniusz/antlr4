...
class ATN(object):
    ...
    def __init__(self, grammarType: ATNType, maxTokenType: int):
        ...
        self.states: t.List[t.Optional[ATNState]] = []
        # Each subrule/rule is a decision point and we must track them so we
        #  can go back later and build DFA predictors for them.  This includes
        #  all the rules, subrules, optional blocks, ()+, ()* etc...
        self.decisionToState: t.List[DecisionState] = []
        # Maps from rule index to starting state number.
        self.ruleToStartState: t.List[ATNState] = []
        # Maps from rule index to stop state number.
        self.ruleToStopState: t.Optional[t.List[ATNState]] = None
        self.modeNameToStartState: t.Dict[str, ATNState] = dict()
        # For lexer ATNs, this maps the rule index to the resulting token type.
        # For parser ATNs, this maps the rule index to the generated bypass token
        # type if the
        # {@link ATNDeserializationOptions#isGenerateRuleBypassTransitions}
        # deserialization option was specified; otherwise, this is {@code null}.
        self.ruleToTokenType: t.Optional[t.List[int]] = None
        # For lexer ATNs, this is an array of {@link LexerAction} objects which may
        # be referenced by action transitions in the ATN.
        self.lexerActions: t.Optional[t.Sequence[t.Optional['LexerAction']]] = None
    ...
    def nextTokensInContext(self, s: ATNState, ctx: t.Optional[RuleContext]):
    ...
    def nextTokens(self, s: ATNState, ctx: t.Optional[RuleContext] = None):
        if ctx == None:
            return self.nextTokensNoContext(s)
        else:
            return self.nextTokensInContext(s, ctx)

    def addState(self, state: t.Optional[ATNState]):
    ...
    #/
    def getExpectedTokens(self, stateNumber: int, ctx: t.Optional[RuleContext]):
